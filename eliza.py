...
# Ruta local para ajedrez
CHESS_PATH = Path(__file__).resolve().parent / "chesso" / "index.html"

# === Disparadores de frases ===
TRIGGER_TTT = re.compile(
    r"(?i)\b(?:hay\s+que\s+jugar|juguemos|quiero\s+jugar|vamos\s+a\s+jugar)\b.*\b("
    r"tic\s*tac\s*toe|gato|tres\s*en\s*raya)\b"
)

TRIGGER_CHESS = re.compile(
    r"(?i)\b(?:hay\s+que\s+jugar|juguemos|quiero\s+jugar|vamos\s+a\s+jugar)\b.*\b("
    r"ajedrez|ajedres|chess)\b"
)
...
def try_open_url(url: str) -> bool:
    """Abre una URL usando distintos métodos para Windows/Linux/Mac."""
    try:
...
class Eliza:
    ...
    def load(self, path):
        """Carga el guion de Eliza desde un archivo (doctor.txt)."""
        key = None
        decomp = None
    ...
    # --- Algoritmo de coincidencia (igual que original) ---
    ...
    def respond(self, text):
        ...
        # === Disparador Tic Tac Toe ===
        if TRIGGER_TTT.search(text):
            candidates = []
            if GAME_URL_ENV:
            ...
            for url in candidates:
                if is_port_open(url):
                    if try_open_url(url):
                        return f"¡Va! Abriendo Tic Tac Toe en {url}"
                    else:
        ...
        # === Disparador AJEDREZ ===
        if TRIGGER_CHESS.search(text):
            if CHESS_PATH.exists():
                try_open_url(CHESS_PATH.as_uri())
                return f"¡Perfecto! Abriendo el juego de ajedrez en: {CHESS_PATH.as_uri()}"
            else:
                return f"No encontré el archivo {CHESS_PATH}. Verifica la ruta."

        # === Lógica normal de Eliza ===
